...
namespace Euler11
{
    public class Program
    {
        ...
        public static int FunctionChain()
        {
            return CrossSelect(ClosedRange(0, 19), ClosedRange(0, 16), (row, col) => Grid[row][col] * Grid[row][col + 1] * Grid[row][col + 2] * Grid[row][col + 3])
                .Concat(CrossSelect(ClosedRange(0, 16), ClosedRange(0, 19), (row, col) => Grid[row][col] * Grid[row + 1][col] * Grid[row + 2][col] * Grid[row + 3][col]))
                .Concat(CrossSelect(ClosedRange(0, 16), ClosedRange(0, 16), (row, col) => Grid[row][col] * Grid[row + 1][col + 1] * Grid[row + 2][col + 2] * Grid[row + 3][col + 3]))
                .Concat(CrossSelect(ClosedRange(0, 16), ClosedRange(3, 19), (row, col) => Grid[row][col] * Grid[row + 1][col - 1] * Grid[row + 2][col - 2] * Grid[row + 3][col - 3]))
                .Max();
        }

        public static IEnumerable<EulerProblemInstance<int>> ProblemInstances
        {
            get
            {
                var factory = EulerProblemInstance<int>.NoParameterInstanceFactory(typeof(Euler11.Program), 11, 70600674);
